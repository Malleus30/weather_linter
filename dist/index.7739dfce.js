...
})({"02dPJ":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "e702ad3b7739dfce";
...
},{}],"6tMK7":[function(require,module,exports) {
var _viewJs = require("./view.js");
var _storageJs = require("./storage.js");
var _dateFns = require("date-fns");
'use strict';
_viewJs.UI.btn.addEventListener('click', change);
_viewJs.UI.heart_picture.style.display = 'none';
_viewJs.UI.heart_picture.addEventListener('click', addLocation);
...
function deleteLocation() {
    ...
    const set = new Set(_storageJs.STORAGE.getFavoredFromStorage());
    set.delete(key);
    _storageJs.STORAGE.setFavoredToStorage([
        ...set
    ]);
}
//dinamic module here
async function change(event) {
    if (event.target.classList.contains('now')) {
        let { show  } = await require("c89ddc9b1ca2a262");
        show(showNow);
    }
...
const render = (data)=>{
    _viewJs.UI.bigCloudIcon.src = `https://openweathermap.org/img/wn/${data.weather[0].icon}@4x.png`;
    _viewJs.UI.bigNumber.textContent = Math.round(data.main.temp - 273);
    _viewJs.UI.bigNumber.textContent += '°';
    _viewJs.UI.showDetailsHeader.textContent = data.name;
    _viewJs.UI.detailsTemp.textContent = `Temperature: ${Math.round(data.main.temp - 273)}°`;
    _viewJs.UI.detailsFeels.textContent = `Feels like: ${Math.round(data.main.feels_like - 273)}°`;
    _viewJs.UI.detailsWeather.textContent = `Weather: ${data.weather[0].main}`;
    _viewJs.UI.detailsSunrise.textContent = `Sunrise: ${timeTransform(data.sys.sunrise)}`;
    _viewJs.UI.detailsSunset.textContent = `Sunset: ${timeTransform(data.sys.sunset)}pm`;
    _viewJs.UI.smalSityName.textContent = data.name;
};
const renderForecast = (data)=>{
    console.log(data.list[0]);
    let result = '';
    _viewJs.FORECAST_UI.showForecast.innerHTML = result;
    ...
    data.list.forEach((item)=>{
       ...
       <div class="date smalText"><p class="date12">${_dateFns.format(new Date(item.dt) * 1000, "MM/dd/yyyy")}</p></div>
    ...
    _viewJs.FORECAST_UI.showForecast.insertAdjacentHTML('beforeend', result);
    _viewJs.FORECAST_UI.showForecast.prepend(paragrath);
};
function addLocation() {
    const set = new Set(_storageJs.STORAGE.getFavoredFromStorage());
    const cityName = _storageJs.STORAGE.getLastlocation();
    set.add(cityName);
    _storageJs.STORAGE.setFavoredToStorage([
        ...set
    ]);
...
function renderFavoritePlaces() {
    const set = new Set(_storageJs.STORAGE.getFavoredFromStorage());
    if (!set) return;
    _viewJs.UI.displayRightDiv.innerHTML = '';
    set.forEach((city)=>{
    ...
    <div class="close">
        ...
        _viewJs.UI.displayRightDiv.append(newLocation);
        addRemovalElement();
...
function fetchRequest(cityName) {
    ...
    _viewJs.UI.heart_picture.style.display = 'block';
    ...
    fetch(url).then((response)=>{
        ...
        render(data);
        _storageJs.STORAGE.setLastLocation(data.name);
    }).catch((error)=>alert(error.message)
    );
    fetch(forecastUrl).then((response)=>{
        return response.json();
    }).then(renderForecast).catch((error)=>alert(error.message)
    );
}
...
},{"./view.js":"2kaIl","./storage.js":"8ubq3","c89ddc9b1ca2a262":"hdydV","date-fns":"9yHCA"}],"2kaIl":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "UI", ()=>UI
);
parcelHelpers.export(exports, "FORECAST_UI", ()=>FORECAST_UI
);
const UI = {
...
const FORECAST_UI = {
    forecastHeader: document.querySelector('.forecastHeader'),
    showForecast: document.querySelector('#showForecast')
};
...
},{}],"8ubq3":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hdydV":[function(require,module,exports) {
module.exports = require("./helpers/browser/js-loader")(require('./helpers/bundle-url').getBundleURL('jPF7t') + "show.8f7c994e.js" + "?" + Date.now()).catch((err)=>{
    delete module.bundle.cache[module.id];
    throw err;
}).then(()=>module.bundle.root('4VxUi')
);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["02dPJ","6tMK7"], "6tMK7", "parcelRequire4363")

//# sourceMappingURL=index.7739dfce.js.map
